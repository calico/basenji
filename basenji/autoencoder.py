...
class AE:
  ...
  def __init__(self, job):
    ...
    self.y = tf.placeholder(
        tf.float32, shape=(self.batch_size, self.num_targets))
    ...
    self.loss = tf.reduce_mean(
        tf.squared_difference(self.y, self.preds), axis=1)
    ...
    # optimization
    ###################################
    self.opt = tf.train.AdamOptimizer(self.learning_rate, self.adam_beta1,
                                      self.adam_beta2, self.adam_eps)
  ...
  def set_params(self, job):
    ...
    self.decoder_units = np.atleast_1d(
        job.get('decoder_units', self.encoder_units))
    ...
    self.decoder_dropout = layer_extend(
        job.get('encoder_dropout', self.encoder_dropout), 0,
        len(self.decoder_units))
