...
class Batcher:
    ...
    def __init__(
        self, Xf, Yf=None, NAf=None, batch_size=64, pool_width=1, shuffle=False
    ):
        self.Xf = Xf
    ...
    def next(self, fwdrc=True, shift=0):
        ...
        if self.start < self.num_seqs:
            ...
            if self.Yf is not None:
                ...
                Yb = np.zeros(
                    (Nb, self.seq_len // self.pool_width, self.num_targets), dtype=ytype
                )
                NAb = np.zeros((Nb, self.seq_len // self.pool_width), dtype="bool")
            ...
            for i in range(Nb):
                ...
                Xb[i] = Xb[i] + (1 / self.seq_depth) * Xbi_n.repeat(
                    self.seq_depth
                ).reshape(self.seq_len, self.seq_depth)
...
class BatcherF:
    ...
    def __init__(
        self, Xf, Yf_real, Yf_imag, NAf=None, batch_size=64, pool_width=1, shuffle=False
    ):
        self.Xf = Xf
        self.num_seqs = self.Xf.shape[0]
        self.seq_len = self.Xf.shape[1]
    ...
    def next(self):
        ...
        if self.start < self.num_seqs:
            ...
            Yb = np.zeros(
                (self.batch_size, self.seq_len // self.pool_width, self.num_targets),
                dtype="float32",
            )
            NAb = np.zeros(
            ...
            for i in range(Nb):
                ...
                for ti in range(self.num_targets):
                    Yb[i, :, ti] = np.fft.irfft(
                        ybi_fourier[:, ti], self.seq_len // self.pool_width
                    )
