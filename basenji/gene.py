...
class TSS:
    def __init__(
        self, identifier, gene_id, chrom, pos, gene_seq, seq_index=False, strand=None
    ):
...
def map_tss_genes(tss_values, tss_list, tss_radius=0):
    if tss_radius > 0:
        print(
            "I don't know how to obtain an exact gene expression measurement with tss_radius > 0",
            file=sys.stderr,
        )
        exit(1)
