...
class GeneData:
  ...
  def __init__(self, genes_hdf5_file, worker_index=None, workers=None):
    ...
    seq_chrom = [
        chrom.decode('UTF-8') for chrom in self.genes_hdf5_in['seq_chrom']
    ]
    ...
    self.transcripts = [
        tx.decode('UTF-8') for tx in self.genes_hdf5_in['transcripts']
    ]
    ...
    for ti in range(len(self.transcripts)):
      self.transcript_map[self.transcripts[ti]] = (transcript_index[ti],
                                                   transcript_pos[ti])
  ...
  def subset_transcripts(self, transcripts):
    ...
    for si in range(self.num_seqs):
      ...
      seq_si_mask = [
          tx_id in transcripts for tx_id, tx_pos in self.seq_transcripts[si]
      ]
      ...
      if np.sum(seq_si_mask) > 0:
        ...
        self.seq_transcripts[si] = [
            self.seq_transcripts[si][sti] for sti in range(len(seq_si_mask))
            if seq_si_mask[sti]
        ]

    # filter the sequence data structures
    self.seq_coords = [
        self.seq_coords[si] for si in range(self.num_seqs) if seq_mask[si]
    ]
    self.seqs_1hot = self.seqs_1hot[seq_mask, :, :]
    self.seq_transcripts = [
        self.seq_transcripts[si] for si in range(self.num_seqs) if seq_mask[si]
    ]
  ...
  def worker(self, wi, worker_num):
    ...
    self.seq_coords = [
        self.seq_coords[si] for si in range(self.num_seqs) if worker_mask[si]
    ]
    self.seq_transcripts = [
        self.seq_transcripts[si] for si in range(self.num_seqs)
        if worker_mask[si]
    ]
    self.num_seqs = len(self.seq_coords)
