...
class SeqNN(seqnn_util.SeqNNModel):
    ...
    def build_from_data_ops(
        ...
        build_rep = lambda do: self.build_predict(
            do[0], do[1], embed_penultimate, target_subset
        )
        self.preds_ensemble = tf.map_fn(
            build_rep, map_elems_eval, dtype=tf.float32, back_prop=False
        )
    ...
    def make_placeholders(self):
        ...
        if self.hp.target_classes == 1:
            self.targets_ph = tf.placeholder(
                ...
                shape=(
                    None,
                    self.hp.seq_length // self.hp.target_pool,
                    self.hp.sum_targets,
                ),
                name="targets",
            )
        ...
        self.targets_na_ph = tf.placeholder(
            tf.bool,
    ...
    def build_predict(
        ...
        if self.hp.augment_mutation > 0:
            ...
            mut_mask_dist = tfp.distributions.Bernoulli(
                probs=mut_mask_probs, dtype=tf.float32
            )
        ...
        if self.hp.nonlinearity == "relu":
            ...
            print(
                'Unrecognized nonlinearity "%s"' % self.hp.nonlinearity, file=sys.stderr
            )
            exit(1)
        ...
        assert self.hp.batch_buffer % pool_preds == 0, (
            "batch_buffer %d not divisible" " by the CNN pooling %d"
        ) % (self.hp.batch_buffer, pool_preds)
            ...
            with tf.variable_scope("final", reuse=tf.AUTO_REUSE):
                ...
                if target_subset is not None:
                    ...
                    filters_full = tf.get_collection(
                        tf.GraphKeys.GLOBAL_VARIABLES, "final/dense/kernel"
                    )[0]
                    bias_full = tf.get_collection(
                        tf.GraphKeys.GLOBAL_VARIABLES, "final/dense/bias"
                    )[0]
                ...
                if self.hp.target_classes > 1:
                    final_repr = tf.reshape(
                        final_repr,
                        (-1, seq_length, self.hp.sum_targets, self.hp.target_classes),
                    )
    ...
    def build_optimizer(self, loss_op):
        ...
        if self.hp.optimizer == "adam":
            ...
            self.opt = tf.train.MomentumOptimizer(
                learning_rate=self.learning_rate_adapt, momentum=self.hp.momentum
            )
        else:
        ...
        self.gvs = self.opt.compute_gradients(
            loss_op, aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N
        )
        ...
        while Xb is not None and (epoch_batches is None or batch_num < epoch_batches):
            ...
            if no_steps:
                run_returns = sess.run(
                    [self.merged_summary, self.loss_train] + self.update_ops,
                    feed_dict=fd,
                )
    ...
    def train_epoch_h5(
        self, sess, batcher, sum_writer=None, epoch_batches=None, no_steps=False
    ):
        """Execute one training epoch using HDF5 data,
    ...
    def train2_epoch_ops(
        self, sess, handle_ph, datasets, sum_writer=None, epoch_batches=None
    ):
        ...
        while gi is not None and (epoch_batches is None or batch_num < epoch_batches):
            try:
                ...
                print(
                    "Genome %d OutOfRangeError with %d sequences remaining."
                    % (gi, datasets[gi].epoch_seqs),
                    file=sys.stderr,
                )
