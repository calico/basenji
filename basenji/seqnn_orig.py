...
class SeqNNOrig(object):
    ...
    def build(self, job):
        ...
        # batches
        self.inputs = tf.placeholder(tf.float32, shape=(self.batch_size, self.seq_length, self.seq_depth), name='inputs')
        if self.target_classes == 1:
            self.targets = tf.placeholder(tf.float32, shape=(self.batch_size, self.seq_length//self.target_pool, self.num_targets), name='targets')
        else:
            self.targets = tf.placeholder(tf.int32, shape=(self.batch_size, self.seq_length//self.target_pool, self.num_targets), name='targets')
        self.targets_na = tf.placeholder(tf.bool, shape=(self.batch_size, self.seq_length//self.target_pool))

        print('Targets pooled by %d to length %d' % (self.target_pool, self.seq_length//self.target_pool))
    ...
    def test(self, sess, batcher, rc=False, shifts=[0], mc_n=0):
        ...
        acc = accuracy.Accuracy(targets, preds, targets_na, batch_losses, batch_target_losses)
