...
class SeqNNModel(object):
    ...
    """
        ...
        for gene_seq in gene_seqs:
            for tss in gene_seq.tss_list:
                tss_pos.add(
                    tss.seq_bin(
                        width=self.hp.target_pool, pred_buffer=self.hp.batch_buffer
                    )
                )
        ...
        for pi in range(self.preds_length):
            ...
            if pi in tss_pos:
                ...
                for ti in range(self.hp.num_targets):
                    grad_piti_op = tf.gradients(
                        self.preds_eval[:, pi, ti],
                        [self.layer_reprs[li] for li in self.grad_layers],
                    )
    ...
    def gradients(self, sess, batcher, rc=False, shifts=[0], mc_n=0, return_all=False):
        ...
        for lii in range(len(self.grad_layers)):
            ...
            lr = np.zeros(
                (batcher.num_seqs, layer_seq_len, layer_units), dtype="float32"
            )
            layer_reprs.append(lr)

            lg = np.zeros(
                (self.hp.num_targets, batcher.num_seqs, layer_seq_len, layer_units),
                dtype="float32",
            )
            ...
            if return_all:
                lra = np.zeros(
                    (batcher.num_seqs, layer_seq_len, layer_units, all_n),
                    dtype="float32",
                )
        ...
        preds = np.zeros(
            (batcher.num_seqs, self.preds_length, self.hp.num_targets), dtype="float32"
        )

        if return_all:
            preds_all = np.zeros(
                (batcher.num_seqs, self.preds_length, self.hp.num_targets, all_n),
                dtype="float32",
            )
        ...
        while Xb is not None:
            ...
            for lii in range(len(self.grad_layers)):
                for ti in range(self.hp.num_targets):
                    layer_grads[lii][ti, si : si + Nb, :, :] = layer_grads_batch[lii][
                        ti, :Nb, :, :
                    ]
                    if return_all:
                        layer_grads_all[lii][
                            ti, si : si + Nb, :, :, :
                        ] = layer_grads_batch_all[lii][ti, :Nb, :, :, :]
        ...
        for lii in range(len(self.grad_layers)):
            ...
            if return_all:
                layer_grads_all[lii] = np.transpose(
                    layer_grads_all[lii], [1, 0, 2, 3, 4]
                )
    ...
    """
        ...
        preds = np.zeros(
            (Xb.shape[0], self.preds_length, self.hp.num_targets), dtype="float32"
        )
        ...
        for ei in range(len(ensemble_fwdrc)):
            ...
            for mi in range(mc_n):
                ...
                preds_ei, layer_reprs_ei = sess.run(
                    [self.preds_train, self.layer_reprs], feed_dict=fd
                )
                ...
                for ti in range(self.hp.num_targets):
                    ...
                    for lii in range(len(self.grad_layers)):
                        ...
                        layer_grads[lii][ti] = self.running_mean(
                            layer_grads_lii_ti1, layer_grads_ti_ei[lii], running_i + 1
                        )
        ...
        if return_var:
            return (
                (preds, preds_var),
                (layer_reprs, layer_reprs_var),
                (layer_grads, layer_grads_var),
            )
        elif return_all:
            return (
                (preds, preds_all),
                (layer_reprs, layer_reprs_all),
                (layer_grads, layer_grads_all),
            )
        else:
    ...
    """
        ...
        for lii in range(len(self.grad_layers)):
            ...
            # gradients
            lg = np.zeros(
                (tss_num, self.hp.num_targets, layer_seq_len, layer_units),
                dtype="float32",
            )
            layer_grads.append(lg)
    ...
    def hidden(self, sess, batcher, layers=None, test_batches=None):
        ...
        batch_num = 0
        while Xb is not None and (test_batches is None or batch_num < test_batches):
            # update feed dict
            fd[self.inputs_ph] = Xb
    ...
    def predict_h5_manual(
        ...
        preds = np.zeros((num_seqs, preds_length, num_targets), dtype=dtype)
        if return_var:
            if all_n == 1:
                print(
                ...
                exit(1)
            preds_var = np.zeros((num_seqs, preds_length, num_targets), dtype=dtype)
        if return_all:
            preds_all = np.zeros(
        ...
        while test_batches is None or batch_num < test_batches:
            ...
            if Xb is None:
                ...
                preds[si : si + Nb, :, :] = preds_batch[:Nb, :, :]
                if return_var:
                    preds_var[si : si + Nb, :, :] = preds_batch_var[:Nb, :, :] / (
                        all_n - 1
                    )
                if return_all:
                    preds_all[si : si + Nb, :, :, :] = preds_batch_all[:Nb, :, :, :]
    ...
    def predict_h5(
        self, sess, batcher, test_batches=None, return_var=False, return_all=False
    ):
        ...
        while test_batches is None or batch_num < test_batches:
            ...
            if Xb is None:
                ...
                if return_var or return_all:
                    preds_batch, preds_ensemble_batch = sess.run(
                        [self.preds_eval, self.preds_ensemble], feed_dict=fd
                    )
    ...
    def predict_tfr(
        self, sess, test_batches=None, sample=1.0, return_var=False, return_all=False
    ):
      ...
      """
        ...
        if sample != 1:
            ...
            sample_pos = np.linspace(
                0, self.preds_length - 1, sample_pos_len, dtype="int"
            )
    ...
    def predict_genes(
        ...
        while not batcher.empty():
            ...
            gseq_preds = self.predict_h5_manual(
                sess,
                batcher,
                rc=rc,
                shifts=shifts,
            ...
            for bsi in range(gseq_preds.shape[0]):
                for tss in gene_seqs[si].tss_list:
                    ...
                    tss_preds[tss_i, :] = gseq_preds[
                        bsi, bi - tss_radius : bi + 1 + tss_radius, :
                    ].sum(axis=0)
                    tss_i += 1
                si += 1
      ...
      """
        ...
        while data_available and (test_batches is None or batch_num < test_batches):
            try:
                ...
                if preds is None:
                    preds = np.zeros(
                        (num_seqs, sample_length, num_targets), dtype="float16"
                    )
                    targets = np.zeros(
                        (num_seqs, sample_length, num_targets), dtype="float16"
                    )
                ...
                if sample_length < self.preds_length:
                    sampled_indexes = np.random.choice(
                        np.arange(self.preds_length), size=sample_length, replace=False
                    )
        ...
        batch_target_losses = np.average(
            batch_target_losses, axis=0, weights=batch_sizes
        )
    ...
    def test_h5_manual(
        self, sess, batcher, rc=False, shifts=[0], mc_n=0, test_batches=None
    ):
        """ Compute model accuracy on a test set.
