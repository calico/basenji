...
tf.flags.DEFINE_boolean(
    "augment_rc", False, "Augment training with reverse complement."
)
tf.flags.DEFINE_boolean(
    "ensemble_rc", False, "Ensemble prediction with reverse complement."
)
tf.flags.DEFINE_string(
    "augment_shifts", "0", "Augment training with shifted sequences."
)
tf.flags.DEFINE_string(
    "ensemble_shifts", "0", "Ensemble prediction with shifted sequences."
)
...
tf.flags.DEFINE_string(
    "logdir", "/tmp/zrl", "directory to keep checkpoints and summaries in"
)
tf.flags.DEFINE_boolean(
    "log_device_placement", False, "Log device placement (ie, CPU or GPU)"
)
tf.flags.DEFINE_integer("seed", 1, "Random seed")


# step counts
tf.flags.DEFINE_integer("train_epochs", None, "Number of training epochs.")
tf.flags.DEFINE_integer(
    "train_epoch_batches", None, "Number of batches per training epoch."
)
tf.flags.DEFINE_integer("test_epoch_batches", None, "Number of batches per test epoch.")
...
# eval options
tf.flags.DEFINE_boolean(
    "metrics_thread", False, "Evaluate validation metrics in a separate thread."
)
...
tf.flags.DEFINE_float(
    "metrics_sample", 1.0, "Sample sequence positions for computing metrics."
)
