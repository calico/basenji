...
################################################################################
def main():
  usage = 'usage: %prog [options] <fasta_file> <seqs_bed_file> <seqs_4C_dir> <tfr_file>'
  ...
  if options.genome_index is None:
    seqs_4C_file = '%s/%d.h5' % (seqs_4C_dir, ti)
  else:
    seqs_4C_file = '%s/%d-%d.h5' % (seqs_4C_dir, options.genome_index, ti)
  while os.path.isfile(seqs_4C_file):
    seqs_4C_files.append(seqs_4C_file)
    ti += 1
    if options.genome_index is None:
      seqs_4C_file = '%s/%d.h5' % (seqs_4C_dir, ti)
    else:
  ...
  if len(seqs_4C_files) == 0:
    print('Sequence coverage files not found, e.g. %s' % seqs_4C_file, file=sys.stderr)
    exit(1)

  seq_pool_len = h5py.File(seqs_4C_files[0], 'r')['seqs_4C'].shape[1]
  num_targets = len(seqs_4C_files)
  ...
  for ti in range(num_targets):
    seqs_4C_open = h5py.File(seqs_4C_files[ti], 'r')
    tii = options.target_start + ti
    targets[:,:,tii] = seqs_4C_open['seqs_4C'][options.start_i:options.end_i,:]
    seqs_4C_open.close()
