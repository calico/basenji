...
def main():
    ...
    parser.add_option(
        "-m", dest="params_file", help="Dimension reduction hyper-parameters file"
    )
    parser.add_option(
        "--mult_cov",
        dest="cov_multiplier",
        default=1,
        type="float",
        help="Coverage multiplier, useful when the read extension and pool width do not match [Default: %default]",
    )
    parser.add_option(
        "-n",
        dest="na_t",
        default=0.25,
        type="float",
        help="Remove sequences with an NA% greater than this threshold [Default: %default]",
    )
    parser.add_option(
        "--no_full",
        dest="no_full",
    ...
    parser.add_option(
        "--scent", dest="scent_file", help="Dimension reduction model file"
    )
    parser.add_option(
        "-t",
    ...
    if (
        options.fourier_dim is not None
        and 2 * options.fourier_dim >= options.seq_length / options.pool_width
    ):
        print(
    ...
    for chrom in chrom_segments:
        segments += [
            (chrom, seg_start, seg_end) for seg_start, seg_end in chrom_segments[chrom]
        ]
    ...
    ################################################################
    seqs_1hot, seqs_segments = segments_1hot(
        fasta_file, segments, options.seq_length, options.stride
    )
    ...
    ################################################################
    print(
        "Reading and pre-processing bigwigs for %d segments" % len(segments), flush=True
    )
    ...
    with tf.Session() as sess:
        ...
        while bstart < len(segments):
            ...
            if options.test_pct_or_chr.startswith("chr"):
                test_bindexes = [
                    twi
                    for twi in range(targets_wig.shape[0])
    ...
    if options.unmap_bed is not None:
        seqs_na = annotate_na(
            seqs_segments, options.unmap_bed, options.seq_length, options.pool_width
        )
        ...
        while bend <= len(seg_values):
            ...
            if pool_width == 1:
                ...
                sv = (
                    ...
                    .reshape((seq_length // pool_width, pool_width))
                    .sum(axis=1)
                )
...
################################################################################
def filter_boring(targets, var_t=0.01):
