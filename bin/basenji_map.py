...
def main():
    ...
    parser.add_option('-n', dest='norm', default=None, type='int', help='Compute saliency norm [Default% default]')
...
def score_write(sess, model, options, seqs_1hot, seqs_chrom, seqs_start):
    ...
    for si in range(seqs_1hot.shape[0]):
        ...
        _, _, _, batch_grads, batch_reprs, _ = model.gradients(sess, batcher, rc=options.rc, shifts=options.shifts, mc_n=options.mc_n, return_all=True)
        print(' Done in %ds.' % (time.time()-t0), flush=True)
