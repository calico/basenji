...
def main():
  ...
  if 'num_targets' not in job or 'target_pool' not in job:
    print(
        "Must specify number of targets (num_targets) and target pooling (target_pool) in the parameters file. I know, it's annoying. Sorry.",
        file=sys.stderr)
    exit(1)
  ...
  batcher = basenji.batcher.Batcher(
      seqs_1hot, batch_size=model.batch_size, pool_width=model.target_pool)
  ...
  with tf.Session() as sess:
    ...
    layer_grads = model.gradients(sess, batcher, options.target_indexes,
                                  options.layers)
