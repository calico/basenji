...
def main():
    ...
    parser.add_option('-l', dest='gene_list', help='Process only gene ids in the given file')
    ...
    with tf.Session() as sess:
        ...
        for si in range(gene_data.num_seqs):
            ...
            batch_grads, batch_reprs = model.gradients_genes(sess, batcher, gene_data.gene_seqs[si:si+1])
            print(' Done in %ds.' % (time.time()-t0), flush=True)
            ...
            for tss_i in range(batch_grads.shape[0]):
                ...
                for tii in range(len(options.target_indexes)):
                    ...
                    batch_grads_score = np.multiply(batch_reprs[0], batch_grads[tss_i,tii,:,:]).sum(axis=1)

                    # open bigwig
                    bw_file = '%s/%s-%s_t%d.bw' % (options.out_dir, tss.gene_id, tss.identifier, ti)
