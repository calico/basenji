...
def main():
    ...
    if len(args) != 3:
        parser.error('Must provide Basenji parameters and model files and test data in HDF5 format.')
    else:
        params_file = args[0]
        model_file = args[1]
        data_file = args[2]
    ...
    job['batch_length'] = test_seqs1.shape[1]
    ...
    # initialize batcher
    if job['fourier']:
        batcher_test = basenji.batcher.BatcherF(test_seqs1, test_targets, test_targets_imag, batch_size=dr.batch_size, pool_width=job['target_pool'])
    else:
        batcher_test = basenji.batcher.Batcher(test_seqs1, test_targets, batch_size=dr.batch_size, pool_width=job['target_pool'])
    ...
    for f in range(num_filters):
        ...
        plot_filter_logo(filter_outs[:,:,f], filter_size, test_seqs, '%s/filter%d_logo'%(options.out_dir,f), maxpct_t=options.act_t)
