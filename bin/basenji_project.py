...
def main():
    ...
    parser.add_option('-d', dest='exp_dir', default='.', help='Experiment directory [Default: %default]')
    parser.add_option('-m', dest='memory', default=6, type='int', help='Memory to look back at [Default: %default]')
    parser.add_option('-o', dest='out_dir', default='proj', help='Output directory [Default: %default]')
    ...
    while os.path.isfile('jobs/%d.txt' % ji):
        ...
        for ei in range(options.memory+1,len(valid_losses)):
            ...
            target_decreases.append((valid_losses[ei-1] - valid_losses[ei]) / valid_losses[ei-1])

            prior_decreases.append([])
            for mi in range(options.memory):
                prior_decreases[-1].append((valid_losses[ei-mi-2] - valid_losses[ei-mi-1])/valid_losses[ei-mi-2])
...
def project_loss(job_losses, model, memory):
    ...
    for mi in range(memory):
        prior_decrease.append( (job_losses[-2-mi] - job_losses[-1-mi]) / job_losses[-2-mi] )
