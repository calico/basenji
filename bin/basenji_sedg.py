...
def main():
  ...
  if 'transcript_targets' in genes_hdf5_in:
    ...
    target_labels = [
        tl.decode('UTF-8') for tl in genes_hdf5_in['target_labels']
    ]
  else:
  ...
  seq_transcripts = [
      seq_transcripts[si] for si in range(len(seq_transcripts)) if seq_mask[si]
  ]
  ...
  with tf.Session() as sess:
    ...
    header_cols = ('rsid', 'index', 'score', 'ref', 'alt', 'gene', 'target',
                   'ref_pred', 'alt pred', 'ser', 'sed')
    if options.csv:
      ...
      for snp_i in seq_snps[soi]:
        ...
        for transcript, tx_pos in sequence_transcripts[soi]:
          ...
          for ti in range(job['num_targets']):
            ...
            snp_gene_sed = grads[tx_pos_buf, alt_nt_i, ti] - grads[tx_pos_buf,
                                                                   ref_nt_i, ti]

            # print to table
            cols = (snp.rsid, snp_is, snp_score, ref_nt, alt_nt, transcript,
                    snp_dist, target_labels[ti], preds[tx_pos_buf, ti],
                    snp_gene_sed)
            if options.csv:
              print(','.join([str(c) for c in cols]), file=sed_out)
            else:
              print(
                  '%-13s %s %5s %6s %6s %12s %5d %12s %6.4f %7.4f' % cols,
                  file=sed_out)
