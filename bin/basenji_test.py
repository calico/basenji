...
def main():
    ...
    model.build_from_data_ops(
        job, data_ops, ensemble_rc=options.rc, ensemble_shifts=options.shifts
    )
    ...
    if options.peaks:
        ...
        for ti in range(test_targets.shape[2]):
            ...
            test_preds_ti_flat = (
                test_preds[:, ds_indexes, ti].flatten().astype("float32")
            )
...
def bigwig_write(bw_file, signal_ti, track_bed, genome_file, buffer=0, bed_set=None):
