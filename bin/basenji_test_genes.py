...
def main():
    ...
    parser.add_option('-l', dest='load_preds', help='Load tss_preds from file')
    ...
    parser.add_option('--table', dest='print_tables', default=False, action='store_true', help='Print big gene/TSS tables [Default: %default]')
    parser.add_option('--tss', dest='tss_alt', default=False, action='store_true', help='Perform alternative TSS analysis [Default: %default]')
    ...
    if options.load_preds is not None:
        ...
        # initialize batcher
        batcher = basenji.batcher.Batcher(gene_data.seqs_1hot, batch_size=model.batch_size)
        ...
        with tf.Session() as sess:
            ...
            # predict
            tss_preds = model.predict_genes(sess, batcher, gene_data.gene_seqs, rc=options.rc, shifts=options.shifts, tss_radius=options.tss_radius)
    ...
    gene_targets = basenji.gene.map_tss_genes(gene_data.tss_targets, gene_data.tss, tss_radius=options.tss_radius)
    gene_preds = basenji.gene.map_tss_genes(tss_preds, gene_data.tss, tss_radius=options.tss_radius)
    ...
    if options.print_tables:
        ...
        gene_table(gene_data.tss_targets, tss_preds, gene_data.tss_ids(), gene_data.target_labels, options.target_indexes, '%s/tss'%options.out_dir, options.plot_scatter)

        gene_table(gene_targets, gene_preds, gene_data.gene_ids(), gene_data.target_labels, options.target_indexes, '%s/gene'%options.out_dir, options.plot_scatter)
    ...
    if options.replicate_labels_file is not None:
        ...
        # replicate_correlations(replicate_lists, gene_data.tss_targets, tss_preds, options.target_indexes, '%s/transcript_reps' % options.out_dir)
    ...
    if options.tss_alt:
        alternative_tss(gene_data.tss_targets[:,options.target_indexes], tss_preds[:,options.target_indexes], gene_data, options.out_dir, log_pseudo=1)
...
def alternative_tss(tss_targets, tss_preds, gene_data, out_base, log_pseudo=1, tss_var_t=1, scatter_pct=0.02):
    ...
    gene_ids = []
    tss_distance = []
    genes_out = open('%s/tss12_genes.txt' % out_base, 'w')
    ...
    for gene_id in gene_tss:
        ...
        if len(tss_list) > 1:
            ...
            # TEMP
            gene_tss1_targets.append(tss_targets_qn[tss_i1,:].astype('float32'))
            gene_tss2_targets.append(tss_targets_qn[tss_i2,:].astype('float32'))
            gene_tss1_preds.append(tss_preds_qn[tss_i1,:].astype('float32'))
            gene_tss2_preds.append(tss_preds_qn[tss_i2,:].astype('float32'))
            gene_ids.append(gene_id)
            print(gene_id, gene_data.tss[tss_i1], file=genes_out)
            print(gene_id, gene_data.tss[tss_i2], file=genes_out)
            tss_distance.append(np.log10(abs(gene_data.tss[tss_i1].pos - gene_data.tss[tss_i2].pos)))
    ...
    for qi in range(len(pct_indexes)):
        ...
        basenji.plots.regplot(gene_tss12_targets[cor_i], gene_tss12_preds[cor_i], out_pdf, poly_order=1, alpha=0.8, point_size=8, square=False, figsize=(4,4), x_label='log2 Experiment TSS1/TSS2', y_label='log2 Prediction TSS1/TSS2', title=gene_ids[cor_i])

        ''' TEMP
        out_pdf = '%s/tss1_q%d.pdf' % (out_base, qi)
        basenji.plots.jointplot(gene_tss1_targets[cor_i], gene_tss1_preds[cor_i], out_pdf, x_label='Experiment log2 TSS1', y_label='Prediction log2 TSS1', cor='pearsonr')

        out_pdf = '%s/tss2_q%d.pdf' % (out_base, qi)
        basenji.plots.jointplot(gene_tss2_targets[cor_i], gene_tss2_preds[cor_i], out_pdf, x_label='Experiment log2 TSS2', y_label='Prediction log2 TSS2', cor='pearsonr')

        out_pdf = '%s/tss12t_q%d.pdf' % (out_base, qi)
        basenji.plots.jointplot(gene_tss1_targets[cor_i], gene_tss2_targets[cor_i], out_pdf, x_label='Experiment log2 TSS1', y_label='Experiment log2 TSS2', cor='pearsonr')

        out_pdf = '%s/tss12p_q%d.pdf' % (out_base, qi)
        basenji.plots.jointplot(gene_tss1_preds[cor_i], gene_tss2_preds[cor_i], out_pdf, x_label='Prediction log2 TSS1', y_label='Prediction log2 TSS2', cor='pearsonr')
        '''
...
def variance_accuracy(gene_targets, gene_preds, out_prefix, log_pseudo=None):
    ...
    if log_pseudo is not None:
        gene_targets = np.log2(gene_targets+log_pseudo)
        gene_preds = np.log2(gene_preds+log_pseudo)
    ...
    basenji.plots.jointplot(gene_mean[ri], gene_std[ri], out_pdf, point_size=10, cor='spearmanr', x_label='Mean across experiments', y_label='Std Dev across experiments')
...
def quantile_accuracy(gene_targets, gene_preds, gene_stat, out_pdf, numq=4):
    ...
    # construct DataFrame
    df_quant = pd.DataFrame({'Quantile':quantiles_series, 'Target':targets_series, 'PearsonR':pcor_series})
    ...
    for qi in range(numq):
        ...
        print('%2d  %.4f  %.4f' % (qi, np.mean(quantile_cors), np.median(quantile_cors)), file=table_out)
    table_out.close()
    ...
    for qi in range(numq):
        ...
        for pqi in range(len(pct_indexes)):
            ...
            # plot_title = 'Target %d / Quantile %d' % (ti,qi)
            # basenji.plots.regplot(gene_targets_quant[:,ti], gene_preds_quant[:,ti], qout_pdf, poly_order=1, alpha=0.8, point_size=8, square=False, figsize=(4,4), x_label='log2 Experiment', y_label='log2 Prediction', title=plot_title)
            basenji.plots.jointplot(gene_targets_quant[:,ti], gene_preds_quant[:,ti], qout_pdf, alpha=0.8, point_size=8, kind='reg', figsize=5, x_label='log2 Experiment', y_label='log2 Prediction')
