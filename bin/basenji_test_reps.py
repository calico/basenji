...
def main():
  ...
  if len(args) != 4:
    parser.error(
        'Must provide parameters, model, test data HDF5, and samples file')
  else:
    params_file = args[0]
    model_file = args[1]
    data_file = args[2]
  ...
  for label in replicate_labels:
    if len(replicate_lists[label]) > 1:
      ...
      test_targets_rep1 = test_targets[:, ds_indexes_targets,
                                       ti1].flatten().astype('float32')
      test_targets_rep2 = test_targets[:, ds_indexes_targets,
                                       ti2].flatten().astype('float32')

      test_preds_rep1 = test_preds[:, ds_indexes_preds, ti1].flatten().astype(
          'float32')
      test_preds_rep2 = test_preds[:, ds_indexes_preds, ti2].flatten().astype(
          'float32')
