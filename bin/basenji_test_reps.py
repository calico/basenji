...
def main():
    usage = (
        "usage: %prog [options] <params_file> <model_file> <data_file> "
        "<samples_file>"
    )
    ...
    if job["fourier"]:
        ...
        batcher_test = basenji.batcher.Batcher(
            test_seqs, test_targets, test_na, dr.batch_size, dr.target_pool
        )
    ...
    for label in replicate_labels:
        if len(replicate_lists[label]) > 1:
            ...
            test_targets_rep1 = (
                test_targets[:, ds_indexes_targets, ti1].flatten().astype("float32")
            )
            test_targets_rep2 = (
                test_targets[:, ds_indexes_targets, ti2].flatten().astype("float32")
            )

            test_preds_rep1 = (
                test_preds[:, ds_indexes_preds, ti1].flatten().astype("float32")
            )
            test_preds_rep2 = (
                test_preds[:, ds_indexes_preds, ti2].flatten().astype("float32")
            )
            ...
            if plot_label:
                ...
                regplot(
                    ...
                    poly_order=1,
                    alpha=0.3,
                    x_label="log2 Replicate 2",
                    y_label="log2 Prediction 2",
                )
            ...
            print(
                "%4d  %4d  %4d  %7.4f  %7.4f  %7.4f  %s"
                % (li, ti1, ti2, cor_reps[-1], cor_wpreds[-1], cor_cpreds[-1], label),
                file=table_out,
            )
...
def jointplot(
    vals1, vals2, out_pdf, alpha=0.5, x_label=None, y_label=None, table=False
):
    ...
    lim_eps = 0.02 * (vmax - vmin)
    ax.text(
        vmax - lim_eps,
        vmin + lim_eps,
        "mean PearsonR %.3f" % vals1.mean(),
