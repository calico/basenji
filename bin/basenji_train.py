...
def run(
    ...
    session_conf = tf.ConfigProto(
        intra_op_parallelism_threads=2, inter_op_parallelism_threads=5
    )
...
def make_data_ops(job, train_patterns, test_patterns):
    ...
    for gi in range(job["num_genomes"]):
        train_dataseq = make_dataset(
            train_patterns[gi], mode=tf.estimator.ModeKeys.TRAIN
        )
        ...
        if train_dataseq.num_targets_nonzero != job["num_targets"][gi]:
            print(
                "WARNING: %s nonzero targets found, but %d specified for genome %d."
                % (train_dataseq.num_targets_nonzero, job["num_targets"][gi], gi),
                file=sys.stderr,
            )
...
def metrics_update(
    epoch, steps, train_losses, valid_losses, valid_accs, time_str, best_str
):
    ...
    print(
        "Epoch: %3d,  Steps: %7d,  Train loss: %7.5f," % (epoch + 1, steps, train_loss),
        end="",
    )
