...
def main():
    ...
    parser.add_option("-l", dest="target_length", default=1024, type="int")
    ...
    for ti in range(targets.shape[2]):
        cols = (
            ...
            target_means[ti],
            target_max[ti],
            targets_df.identifier[ti],
            targets_df.description[ti],
        )
    ...
    plot_distr_args = []
    for ti in range(targets.shape[2]):
        targets_ti = np.random.choice(
            targets[:, :, ti].flatten(), size=10000, replace=False
        )
